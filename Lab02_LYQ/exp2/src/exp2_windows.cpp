/**
 * Lab02 实验二 - Windows 版本（供 VTune 分析用）
 *
 * 编译（MinGW-w64）：
 *   g++ -std=c++11 -O2 -march=native -g -fno-omit-frame-pointer -o exp2_vtune.exe exp2_win.cpp
 *
 * 用法：
 *   exp2_vtune.exe perf  <n> <runs> <algo>
 *   exp2_vtune.exe float <n>
 *
 * 输出：algo,n,runs,time_ms,result
 */
...
float fsum_unroll2(float *a, int n) {
    float t[2]={0}; for(int i=0;i<n;i+=2){t[0]+=a[i];t[1]+=a[i+1];}
    return common_algo_f(t,2);
}
float fsum_unroll4(float *a, int n) {
    float t[4]={0}; for(int i=0;i<n;i+=4){t[0]+=a[i];t[1]+=a[i+1];t[2]+=a[i+2];t[3]+=a[i+3];}
    return common_algo_f(t,4);
}
float fsum_unroll8(float *a, int n) {
    float t[8]={0};
    for(int i=0;i<n;i+=8){t[0]+=a[i];t[1]+=a[i+1];t[2]+=a[i+2];t[3]+=a[i+3];
                           t[4]+=a[i+4];t[5]+=a[i+5];t[6]+=a[i+6];t[7]+=a[i+7];}
...
int main(int argc, char* argv[]) {
    ...
    if (!strcmp(argv[1], "perf")) {
        ...
        printf("%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.6f,%.2f\n",
               n, r_naive,r_unroll2,r_unroll4,r_unroll8,r_unroll16,r_unroll32,(float)ref,max_err);
        free(a);
    } else {
